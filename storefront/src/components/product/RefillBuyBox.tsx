...
export function RefillBuyBox({ product }: RefillBuyBoxProps) {
  ...
  const batchCode = `CRT-${product.category.toUpperCase().slice(0, 3)}-2026-08B`
  ...
  const handleAddToCart = () => {
    for (let i = 0; i < quantity; i++) {
      addItem({
        ...
        sku: `PEP-CRT-${product.id.replace("cartridge-", "").toUpperCase()}`,
        batch: batchCode,
        image: product.image,
        options: [
          { label: "Purchase Type", value: purchaseType === "subscription" ? "28-Day Subscription (10% off)" : "One-Time Order" },
          { label: "Format", value: "1.5ml Refill Cartridge" },
          { label: "Compatibility", value: "PEPTECH® Precision Pen System" },
        ],
      })
    }
  ...
  return (
    <div className="flex flex-col gap-[18px] items-start w-full max-w-[600px]">
      ...
      {/* Category / Format Pill & Rating */}
      <div className="flex items-center justify-between w-full">
        <span className="bg-[#e6fffa] text-[#0d7b78] border border-[#99f6e4] text-[11px] font-bold px-2.5 py-1 rounded-[4px] uppercase tracking-wider">
          {product.formatLabel}
        </span>
      ...
      <div className="flex flex-col gap-[4px] items-start w-full">
        ...
        <p className="font-semibold text-[#16a6a3] text-[14px]">
          {product.categoryLabel} • Laboratory Reference Standard
        </p>
      </div>

      {/* Description */}
      <p className="font-normal text-[#475569] text-[13px] leading-[22px]">
        {product.description}. Formulated specifically for the PEPTECH® reusable precision pen system. Laboratory analytical grade with lot-specific HPLC and Mass Spectrometry validation.
      </p>

      {/* Mandatory Rule 2 Pen Compatibility Banner */}
      <div className="bg-[#f0f6fa] border border-[#d2e2ec] rounded-[10px] p-3.5 w-full flex items-start gap-3">
        <div className="size-[20px] rounded-full bg-[#16a6a3] text-white flex items-center justify-center shrink-0 mt-0.5 text-[11px] font-bold">
          ✓
        </div>
        <div className="flex-1 text-[12px] text-[#0b1f3a]">
          <p className="font-semibold">Exclusively fits PEPTECH® Precision Pens</p>
          <p className="text-[#475569] mt-0.5">
            Designed for exact dial volume calibration. Don&apos;t have the pen system yet?{" "}
            <Link href="/pen-sets" className="font-bold text-[#16a6a3] hover:underline">
              Get the Complete Pen Set →
            </Link>
          </p>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-[10px] items-start w-full">
        ...
        {/* Option 1: Subscribe & Save 10% (Default for Refills) */}
        <div
          ...
          <div className="flex gap-[12px] items-center">
            ...
            <div className="flex flex-col gap-[2px] items-start">
              ...
              <span className="text-[#64748b] text-[12px]">
                Ships every 28 days • Pause or cancel anytime
              </span>
            </div>
          </div>
      ...
      <div className="border-t border-[#f1f5f9] flex items-start justify-between pt-[14px] w-full">
        <div className="flex flex-col gap-[3px] items-center text-center w-[135px]">
          ...
          <span className="font-normal text-[#64748b] text-[10px]">Zero peptide branding</span>
        </div>
        <div className="flex flex-col gap-[3px] items-center text-center w-[135px]">
        ...
        <div className="flex flex-col gap-[3px] items-center text-center w-[135px]">
          ...
          <span className="font-bold text-[#0b1f3a] text-[12px]">HPLC Verified</span>
          <span className="font-normal text-[#64748b] text-[10px]">≥99.0% Certified</span>
        </div>
        <div className="flex flex-col gap-[3px] items-center text-center w-[135px]">
